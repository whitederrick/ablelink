...
describe("decideChargeOutcome — HTTP 오류(Toss 응답 확정)", () => {
  it("5xx + 유예 이내 → 재시도", () => {
    expect(decideChargeOutcome({ kind: "http_error", status: 500 }, 1, GRACE)).toEqual({
      action: "retry",
      wipeBillingKey: false,
    });
  });
  it("429 + 유예 이내 → 재시도", () => {
    expect(decideChargeOutcome({ kind: "http_error", status: 429 }, 2, GRACE)).toEqual({
      action: "retry",
      wipeBillingKey: false,
    });
  });
  it("5xx + 유예 초과 → 강등 + 키삭제", () => {
    expect(decideChargeOutcome({ kind: "http_error", status: 503 }, 3, GRACE)).toEqual({
      action: "downgrade",
      wipeBillingKey: true,
    });
  });
  it("카드 거절(4xx) → 유예 이내여도 즉시 강등 + 키삭제(확정 실패)", () => {
    expect(decideChargeOutcome({ kind: "http_error", status: 400 }, 0, GRACE)).toEqual({
      action: "downgrade",
      wipeBillingKey: true,
    });
  });
