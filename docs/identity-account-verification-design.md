...
>
> **✅ 결정(2026-06-18): 결제/빌링 = 토스페이먼츠 유지, 본인인증·계좌 예금주조회 = 포트원(PortOne).**
> - 포트원 **Free 티어**(월 순거래 5천만원 미만, 플랫폼 이용료 0원, 본인인증 연동 무제한 포함)로 시작.
> - 인증 1건당 **인증사 단가만** 비용(휴대폰 ~40원, 계좌조회 ~50원 — 어느 벤더든 동일한 규제 비용).
> - 본인인증·계좌인증은 규제 서비스라 **인증사 채널 활성화(신청)** 가 별도로 필요(셀프 즉시 X).
> - 코드는 `provider` 추상화라 포트원 1곳만 `callProvider()`에 붙이면 P1·P3 동시 가동.
