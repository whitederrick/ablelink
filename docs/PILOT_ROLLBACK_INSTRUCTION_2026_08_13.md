...
| 항목 | 처리 |
|---|---|
| 파일럿 전용 문서 화면 | **미리보기·다운로드 + 사업체 담당자 서명**(★2026-08-23 개정, 아래 2-3) |
...
### 2-3. ★개정(2026-08-23) — 사업체 담당자 서명 추가

파일럿 수행 중 사용자 요청으로 **대면 전자서명(담당자에게 폰 건네기)** 을 파일럿 화면에 추가했다.
제출·발송은 여전히 **만들지 않는다** — 서명은 PDF 안에 들어갈 뿐 어디로도 나가지 않는다.

| 항목 | 처리 |
|---|---|
| 화면 | `app/pilot/docs/sign` 신설. `/pilot/docs` 에 서명 섹션(미서명/서명 완료 배지) |
| 업로드 | ★**운영 `POST /api/worker/docs/inperson-sign` 을 그대로 재사용**한다. 새 업로드 라우트를 만들지 않았다 — 이미지 검증·Storage 경로·토큰 발급 규율이 두 벌로 갈리는 것을 막는다 |
| 운영 코드 변경 | **0** — 운영 파일은 한 줄도 고치지 않았다. 운영 서명 화면(`/worker/docs/manager-sign`)을 재사용하지 않은 유일한 이유는 그 화면이 저장 후 `/worker/docs`(제출·발송이 있는 곳)로 돌아가기 때문이다 |
| 조회 | `findPilotCompanySignature` — (배정 + 기간)의 `company_manager` 사용 완료 토큰 중 **최신**. 운영 `buildDocPayload:127-142` 폴백과 같은 규칙이며 `docType` 은 비교하지 않는다(한 번 서명 → 같은 기간 출근부·훈련일지 공용) |
| 배지 | `GET /api/pilot/docs/sign-status` 가 **PDF 와 같은 함수**로 판정한다 — 화면과 문서가 어긋날 수 없다 |
| 슬롯 없는 문서 | 적응지도 일지는 서명 2행뿐이라 **조회조차 하지 않는다**(`PILOT_DOCS_WITH_COMPANY_SIGN`) |
| 초기화 영향 | **없다.** `SiteSignToken` 은 배정 Cascade, Storage 는 `inperson/{assignmentId}/` prefix 나열이 이미 회수한다(§7·§10-2). 업로드 라우트도 배포 게이트에서 이미 검토된 3개 중 하나다 |
| 검증 | `verify-pilot-docs.mts` 53/53 — 토큰 선택 규칙 6건 + **HTTP end-to-end**(서명 전후 PDF 바이트 증가·양성 대조·404·401) |
...
| 서명 이미지 | ~~만들지 않는다~~ → ★**2026-08-23 개정**: 파일럿 화면에서 대면 서명을 받는다(§2-3). 받았으면 그 토큰의 `signerName` 이 `businessContactName` 보다 **우선**한다(운영과 같은 규칙) |
