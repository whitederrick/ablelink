...
>
> 이 문서는 **직무지도원 자가 프로비저닝 모델**의 과거 v3 설계이며 **현재 운영 모델과 일치하지 않습니다.**
> 현행은 시스템 운영자가 셋업하고 직무지도원은 일반 사용자로 사용하는 모델이며,
> 격리 방식도 `Agency.isDemo` + Site 쪼개기가 아니라 `PilotSession` 회차 + 명시 FK입니다.
>
> **최종 기준 문서: `docs/PILOT_DESIGN_2026_08_12_v1_7.md`**
>
> 본 문서는 **의사결정 이력 보존 목적으로만** 유지합니다.
> 이 문서의 유효한 코드 실측 내용은 전부 v1.7과 프로젝트 메모리로 이관되었습니다.

---

# 직무지도원 문서 파일럿 서비스 설계 v3 — Claude 2차 검토 반영안 (폐기)
...
검증 항목은 **전 케이스에서 확인문구·작성일·서명 3행이 같은 페이지에 있는지**다.

★**baseline은 관측값 그대로 기록한다(2026-08-11 확정).**
2026-07-20 스윕은 당시 `3792360` 수정 후 분리 0건이었으나, 그 이후 코드가 바뀌었을 수 있으므로
**이번 P0 실행 결과가 0건이라고 전제하지 않는다.** 0건이 아니어도 그것이 현재 코드의 사실이며,
0건을 만들려고 P0에서 코드를 고치지 않는다(P0은 애플리케이션 코드 변경 금지 범위다).
따라서 **P6·P8의 통과 기준은 "분리 0건"이 아니라 "P0 baseline 대비 악화 없음"** 이다.
baseline이 0이 아닌 것으로 관측되면 그 자체를 별도 이슈로 기록하고 P6 범위와 분리해 판단한다.
