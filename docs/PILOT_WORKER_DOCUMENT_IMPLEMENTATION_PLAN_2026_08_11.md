# 직무지도원 문서 파일럿 구현 계획 v3 — Claude 2차 검토 반영안
...
- 개편 전 스윕을 1회 실행해 **기준 출력 확보**(서명 블록 분리 0건 baseline)
...
- **스윕 스크립트 동작 확인 + 519케이스 개편 전 baseline 확보(분리 0건)**
- PDF 5종 기준 출력 보관
...
- Word/HWP API·버튼 없음
- **Vitest 416건 무회귀**
- **PDF 기간 스윕 519케이스 재실행 — 서명 블록 분리 0건(P0 baseline과 동일)**
- PDF 5종 시각 회귀검증 통과
