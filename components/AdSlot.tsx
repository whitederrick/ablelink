...
export default function AdSlot({
  ...
  const card = (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-emerald-100 bg-emerald-50/60">
      {c.imageUrl && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={c.imageUrl} alt="" className="h-20 w-full flex-shrink-0 object-cover" />
      )}
      <div className="flex flex-1 flex-col p-3">
        <span className="mb-1 w-fit rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700">{c.badge ?? label}</span>
        <h3 className="text-sm font-black leading-tight text-slate-900">{c.title}</h3>
        {c.description && <p className="mt-0.5 line-clamp-2 text-xs font-semibold leading-relaxed text-slate-500">{c.description}</p>}
        {/* 로테이션 인디케이터(광고 2개 이상) */}
        {contents.length > 1 && (
          <div className="mt-auto flex items-center gap-1 pt-2">
            {contents.map((_, i) => (
              <button
                ...
                aria-label={`광고 ${i + 1}`}
                className={`h-1.5 rounded-full transition-all ${i === idx % contents.length ? "w-4 bg-slate-700" : "w-1.5 bg-slate-300 hover:bg-slate-400"}`}
              />
            ))}
          </div>
        )}
