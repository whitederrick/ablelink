...
// 개인정보처리방침 본문(조문) — 단일 출처.
// /privacy 전체 페이지와 앱 내 모달(LegalDocModal)이 공유한다. 방침 수정은 여기 한 곳만.
// 순수 표시 컴포넌트(훅 없음) → 서버 페이지·클라이언트 모달 양쪽에서 렌더 가능.
...
export function PrivacyPolicyContent() {
  return (
    <div className="space-y-8 text-sm font-semibold leading-7 text-slate-700">
      ...
      <section>
        <h2 className="mb-3 text-base font-black text-slate-900">제5조의2 (개인정보의 국외 이전)</h2>
        <p>회사는 <strong>AI 음성 일지 작성(선택 기능)</strong>을 제공하기 위해, 이용자가 해당 기능을 사용하는 경우에 한하여 아래와 같이 개인정보를 국외로 이전(처리위탁)합니다. 이용자는 이 국외 이전에 대해 <strong>별도로 동의</strong>한 경우에만 음성 입력 기능(음성 원문 변환·AI 문장 생성)을 이용할 수 있으며, 동의하지 않아도 <strong>직접 입력(키보드)</strong> 등 다른 방식으로 일지를 작성할 수 있습니다.</p>
        <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white">
          <table className="w-full text-xs">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-3 py-2.5 text-left font-black text-slate-900">이전받는 자</th>
                <th className="px-3 py-2.5 text-left font-black text-slate-900">국가</th>
                <th className="px-3 py-2.5 text-left font-black text-slate-900">이전 항목</th>
                <th className="px-3 py-2.5 text-left font-black text-slate-900">이용 목적</th>
                <th className="px-3 py-2.5 text-left font-black text-slate-900">보유·이용 기간</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 align-top">
              <tr>
                <td className="px-3 py-2.5">Groq, Inc.</td>
                <td className="px-3 py-2.5">미국</td>
                <td className="px-3 py-2.5">음성 녹음 파일(발화 내용에 훈련생 정보가 포함될 수 있음)</td>
                <td className="px-3 py-2.5">음성 인식(음성→텍스트 변환)</td>
                <td className="px-3 py-2.5">변환 처리 즉시 파기(회사는 미보관). 이전받는 자의 자체 정책에 따른 일시적 처리 로그는 해당 사업자 정책을 따름</td>
              </tr>
              <tr>
                <td className="px-3 py-2.5">Google LLC (Gemini)</td>
                <td className="px-3 py-2.5">미국</td>
                <td className="px-3 py-2.5">변환된 텍스트 및 일지 작성 맥락(훈련생 성명, 현장·과제 정보 등)</td>
                <td className="px-3 py-2.5">업무일지 문장 생성</td>
                <td className="px-3 py-2.5">변환 처리 즉시 파기(회사는 미보관). 이전받는 자의 자체 정책에 따른 일시적 처리 로그는 해당 사업자 정책을 따름</td>
              </tr>
            </tbody>
          </table>
        ...
        <ul className="mt-3 list-disc space-y-1.5 pl-5">
          <li><strong>이전 일시 및 방법:</strong> 이용자가 AI 음성 기능을 실행하는 시점에, 암호화된 통신(HTTPS)을 통해 위 사업자의 API로 실시간 전송</li>
          <li><strong>이전을 거부할 권리 및 방법:</strong> 이용자는 국외 이전을 거부할 수 있으며, 동의하지 않으면 음성 입력 기능(음성 원문 변환·AI 문장 생성)만 제한될 뿐 서비스의 다른 기능 이용에는 영향이 없습니다. 이미 한 동의는 프로필의 문의 경로 또는 고객센터를 통해 철회할 수 있습니다.</li>
        </ul>
      </section>
