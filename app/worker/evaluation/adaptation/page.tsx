...
// 취업 후 적응지도 대상자 종합 평가기록부
// evalType = "ADAPTATION", label1 = "초기", label2 = "후기"
...
function AdaptationEvalInner() {
  const router = useRouter();
  const params = useSearchParams();
  const traineeId   = params.get("traineeId") || "";
  const traineeName = params.get("traineeName") || "대상자";
  ...
  useEffect(() => {
    ...
    fetch(`/api/worker/evaluation?traineeId=${traineeId}&evalType=ADAPTATION&periodStart=${periodStart}&periodEnd=${periodEnd}`)
      .then(r => r.json())
      .then(d => {
  ...
  async function save() {
    ...
    const res = await fetch("/api/worker/evaluation", {
      ...
      body: JSON.stringify({ traineeId, evalType: "ADAPTATION", periodStart, periodEnd, scores, comments }),
    });
  ...
  return (
    <div className="min-h-dvh bg-slate-50 pb-20">
      <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-slate-100 bg-white px-4 py-4">
        ...
        <div>
          <h1 className="text-base font-black text-slate-900">취업 후 적응지도 종합 평가기록부</h1>
      ...
      <div className="mx-auto max-w-md space-y-3 px-4 py-3">
          ...
          <div key={sec.code} className="rounded-2xl border border-slate-100 bg-white p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-xs font-semibold text-slate-400">
                초기: {sec.items.reduce((s, _, i) => s + (parseInt(scores[sec.code]?.[i]?.initial) || 0), 0)}점 /
                후기: {sec.items.reduce((s, _, i) => s + (parseInt(scores[sec.code]?.[i]?.final) || 0), 0)}점
              </span>
            </div>
              ...
              {sec.items.map((item, idx) => (
                <div key={idx} className="border-b border-slate-50 pb-3 last:border-b-0 last:pb-0">
                  ...
                  <div className="flex gap-4">
                    {[
                      { label: "초기", field: "initial" as const },
                      { label: "후기", field: "final" as const },
                    ].map(({ label, field }) => (
        ...
        <button onClick={save} disabled={saving || !traineeId}
          className="min-h-14 w-full rounded-2xl bg-violet-700 text-base font-black text-white shadow-lg transition active:scale-[0.97] disabled:opacity-60">
          {saving ? "저장 중..." : "저장"}
        </button>
      </div>
